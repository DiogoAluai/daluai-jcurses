...
/**
 * This class is used by widgets, having scrollable content, to paint scrollbars. The class <code>jcurses.util.ScrollbarUtils is deprecated. 
 *
 */
public class ScrollbarPainter
{
  ...
  /**
   * This method is to be called if the scrollable content ( or it's position ) has been ( or could have been ) changed, to refresh scrollbars only if needed.
   *  
   */
  public void refresh()
  {
  ...
  private ScrollbarData readData()
  {
    ...
    {
      result.horizontalOffset = Math.round(horizontalLength * _widget.getHorizontalScrollbarOffset());
      result.horizontalLength = Math.round(horizontalLength * _widget.getHorizontalScrollbarLength());
    ...
    if ( _widget.hasVerticalScrollbar() && ( _widget.getVerticalScrollbarLength() > 0 ) )
    {
      ...
      result.verticalOffset = Math.round(verticalLength * _widget.getVerticalScrollbarOffset());
      result.verticalLength = Math.round(verticalLength * _widget.getVerticalScrollbarLength());
  ...
  private void drawHorizontalScrollbar()
  {
    ...
    {
      Toolkit.drawHorizontalThickLine(_borderRectangle.getX() + 1 + offset, _borderRectangle.getY() + _borderRectangle.getHeight() - 1, _borderRectangle.getX()
          + offset + length, _widget.getScrollbarColors());
    }
  }
  ...
  private void drawVerticalScrollbar()
  {
    ...
    {
      Toolkit.drawVerticalThickLine(_borderRectangle.getX() + _borderRectangle.getWidth() - 1, _borderRectangle.getY() + 1 + offset, _borderRectangle.getY()
          + offset + length, _widget.getScrollbarColors());
    }
  }
  ...
  private void refreshVerticalScrollbar(ScrollbarData old)
  {
    if ( _widget.hasVerticalScrollbar() )
    {
      ...
      {
        Toolkit.drawVLineClip(_borderRectangle.getX() + _borderRectangle.getWidth() - 1, _borderRectangle.getY() + 1 + old.verticalOffset, _borderRectangle
            .getY()
            + old.verticalOffset + old.verticalLength, _widget.getBorderColors());
      }
      drawVerticalScrollbar();
    }
  }
  ...
  private void refreshHorizontalScrollbar(ScrollbarData old)
  {
    if ( _widget.hasHorizontalScrollbar() )
    {
      ...
      {
        Toolkit.drawHLineClip(_borderRectangle.getX() + 1 + old.horizontalOffset, _borderRectangle.getY() + _borderRectangle.getHeight() - 1,
  ...
  private static class ScrollbarData
  {
    ...
    {
      return "hoffset=" + horizontalOffset + ",hlength=" + horizontalLength + ",voffset=" + verticalOffset + ",vlength=" + verticalLength;
    }
  }
